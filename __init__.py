...
def parse(pattern, source):
    ...
    try:
        ...
        if not stream.eof:
            ...
            print stream.extreme
            print stream.get(stream.pos-4)
            print stream.get(stream.pos-3)
            print stream.get(stream.pos-2)
            print stream.get(stream.pos-1)
            print '-->', stream.get(stream.pos)
            print stream.get(stream.pos+1)
            print stream.get(stream.pos+2)
            print stream.get(stream.pos+3)
            print stream.get(stream.pos+4)
...
indent  = TokenTerminal('indent', True)
newline = TokenTerminal('newline', True)
dedent  = TokenTerminal('dedent', True)

string_begin  = TokenTerminal('string_begin', True)
string_data   = TokenTerminal('string_data')
string_escape = TokenTerminal('string_escape', True)
